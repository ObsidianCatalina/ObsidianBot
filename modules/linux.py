...
async def linux(app: Ariadne, group: Group,event:GroupMessage):
 if event.sender.id == su:
    ls = [
        ...
        "5，sudo rm -rf file1 file2 file3或者sudo rm -rf folder1 folder2 folder3等\n",
        "6，不同的系统拥有不同的包管理，如deb系统的包管理为apt，dpkg RHEL系统为yum，dnf（openSUSE为zypper），Arch系统主要为pacman等\n",
        "7，mkdir命令主要用于创建文件夹/目录，用法为mkdir <folder name>\n",
        "8，tar命令的参数有很多，如-c,-x,-t,-z,-j,-v,-f,-p,-P,-N,-exclude FILE等",
    ]
    await app.send_message(
        group,
        MessageChain((ls))
